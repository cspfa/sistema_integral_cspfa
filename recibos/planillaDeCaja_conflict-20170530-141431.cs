...
namespace SOCIOS
{
    public partial class planillaDeCaja : Form
    {
        ...
        private void cargaInicial()
        {
            ...
            if (VGlobales.vp_role == "CAJA" || VGlobales.vp_role == "CAJA2")
            {
                buscar("0", dgOtros);
                buscar("E", dgEgresos);
                buscar("2", dgCheques);
        ...
        private void comboBancos()
        {
            ...
            cbBancos.DataSource = dlog.BO_EjecutoDataTable(query);
            cbBancos.DisplayMember = "NOMBRE";
            cbBancos.ValueMember = "ID";
            cbBancos.SelectedItem = 0;

            cbBancosCheques.DataSource = null;
            cbBancosCheques.Items.Clear();
            cbBancosCheques.DataSource = dlog.BO_EjecutoDataTable(query);
            cbBancosCheques.DisplayMember = "NOMBRE";
            cbBancosCheques.ValueMember = "ID";
            cbBancosCheques.SelectedItem = 0;
        }
        ...
        {
            decimal CAMBIO = 0;
            string query = "SELECT VALOR FROM CONFIG WHERE PARAM = 'CAMBIO' AND ROL = CURRENT_ROLE;";
            DataRow[] foundRows;
            foundRows = dlog.BO_EjecutoDataTable(query).Select();
        ...
        private void buscar(string PAGO, DataGridView GRID)
        {
            try
            {
                ...
                if (VGlobales.vp_role == "CAJA" || VGlobales.vp_role == "INFORMES" || VGlobales.vp_role == "CAJA2")
                    query = "SELECT * FROM PLANILLA_CAJA ('" + PAGO + "') WHERE DESTINO IS NULL OR DESTINO <> 4 OR DESTINO <> 2;";
                
                if (VGlobales.vp_role == "SSADPADUA")
                    query = "SELECT * FROM PLANILLA_CAJA ('" + PAGO + "') WHERE DESTINO = 4;";

                if (VGlobales.vp_role == "CPOCABA")
                    query = "SELECT * FROM PLANILLA_CAJA ('" + PAGO + "') WHERE DESTINO = 9;";
                ...
                using (FbConnection connection = new FbConnection(connectionString))
                {
                    ...
                    dt1.Columns.Add("IMPUTACION", typeof(int));
                    ...
                    while (reader.Read())
                    {
                        ...
                        RECIBO = reader.GetString(reader.GetOrdinal("RECIBO")).Trim();
                        ...
                        F_PAGO = reader.GetString(reader.GetOrdinal("F_PAGO"));
                        dt1.Rows.Add(RECIBO, DETALLE, CONCEPTO, IMPUTACION, VALOR, OBSERVACIONES, FECHA, ANULADO, F_PAGO);

                        if (PAGO == "2")
                        {
                            dgComposicion.Rows.Add(RECIBO.Replace("R",""), "CHEQUE " + CONCEPTO + " " + FECHA, string.Format("{0:n}", (IMPORTE)));
                        }
                    }
        ...
        private void cerrar()
        {
            ...
            foreach (DataGridViewRow row in dgEfectivo.Rows)
            {
                ...
                if (NRO.Contains("R"))
                {
                    try
                    {
                        int ID_RECIBO = int.Parse(NRO.Replace("R", ""));
                        BO_CAJA.cajaEnRecibos(ID_RECIBO, CAJA_DIARIA);
                    }
                    catch (Exception error)
                    {
                ...
                else if (NRO.Contains("B"))
                {
                    try
                    {
                        int ID_BONO = int.Parse(NRO.Replace("B", ""));
                        BO_CAJA.cajaEnBonos(ID_BONO, CAJA_DIARIA);
                    }
                    catch (Exception error)
                    {
            ...
            foreach (DataGridViewRow row in dgEgresos.Rows)
            {
                string ID = row.Cells[0].Value.ToString().Replace("R", "");
                string F_PAGO = row.Cells[8].Value.ToString();

                if (F_PAGO == "2")
                {
                    BO_CAJA.depositarChequeAlCierre(int.Parse(ID), 2, CAJA_DIARIA);
                }
        ...
        private DataSet buscarCajasDepositadas(int CAJA)
        {
            ...
            using (FbConnection connection = new FbConnection(connectionString))
            {
                ...
                FbTransaction transaction = connection.BeginTransaction();
                DataSet ds = new DataSet();
                string query = "SELECT C.FECHA, C.TOTAL, C.IMPUTACION, B.NOMBRE FROM CAJA_DIARIA C, BANCOS B WHERE C.CAJA_DEPOSITADA = " + CAJA + " AND C.BANCO = B.ID;";
                FbCommand cmd = new FbCommand(query, connection, transaction);
                cmd.CommandText = query;
                cmd.Connection = connection;
                cmd.CommandType = CommandType.Text;
        ...
        private DataSet buscarChequesInforme(int CAJA)
        {
            ...
            using (FbConnection connection = new FbConnection(connectionString))
            {
                ...
                FbTransaction transaction = connection.BeginTransaction();
                DataSet ds = new DataSet();
                string query = "SELECT R.ID, R.OBSERVACIONES, R.FECHA_RECIBO, R.VALOR, R.CUENTA_DEBE  FROM CAJA_DIARIA_CHEQUES C, RECIBOS_CAJA R WHERE R.ID = C.CHEQUE AND C.CAJA_DIARIA = " + CAJA + ";";
                FbCommand cmd = new FbCommand(query, connection, transaction);
                cmd.CommandText = query;
                cmd.Connection = connection;
                cmd.CommandType = CommandType.Text;
        ...
        private DataSet buscarIngresosInforme(int CAJA, string PAGO)
        {
            ...
            using (FbConnection connection = new FbConnection(connectionString))
            {
                ...
                if (VGlobales.vp_role == "SSADPADUA")
                    query = "SELECT * FROM PLANILLA_CAJA_INFORME ('" + PAGO + "', " + CAJA + ") WHERE DESTINO = 4;";
            ...
            try
            {
                ...
                if (CAJAS_DEPOSITADAS.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow row in CAJAS_DEPOSITADAS.Tables[0].Rows)
                    {
                        ...
                        string TOTAL_CAJA = row[1].ToString();
                        ...
                        PdfPCell CELL_TOTAL_CAJA = new PdfPCell(new Phrase("$ " + TOTAL_CAJA, _mediumFont));
                        CELDA_TOTAL_CAJA.HorizontalAlignment = 2;
                ...
                decimal SALDO_ANTERIOR = decimal.Parse(dgCajasAnteriores[9, dgCajasAnteriores.CurrentCell.RowIndex + 1].Value.ToString());
        ...
        private void btnAgregarCheque_Click(object sender, EventArgs e)
        {
            ...
            {
                COMP = int.Parse(tbCompCheque.Text);

                try
                {
                    foreach (DataGridViewRow row in dgCheques.SelectedRows)
                    {
                        ID = int.Parse(row.Cells[0].Value.ToString().Replace("R", ""));
        ...
        private void btnQuitarCheque_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string IDE = row.Cells[0].Value.ToString().Replace("R", "");
                    IDE = IDE.Replace("B", "");
                    ...
                    if (F_PAGO == "CHEQUE")
                    {
                        BO_CAJA.depositarChequeAlCierre(ID, 0, 0);
                        cargaInicial();
                    }
                    else
        ...
        private void dgCajasAnteriores_MouseUp(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                int CAJAS = dgCajasAnteriores.SelectedRows.Count;

                if (CAJAS == 1)
                { 
                    btnDepositarCaja.Text = "DEPOSITAR CAJA SELECCIONADA";
                }

                if (CAJAS > 1)
                {
                    btnDepositarCaja.Text = "DEPOSITAR CAJAS SELECCIONADAS";
                }

                cmsCajas.Show(Cursor.Position);
            }
        }
        ...
        private void btnDepoAllCajas_Click(object sender, EventArgs e)
        {
            int CAJA = 0;
            string DEPOSITADA = "";
            int BANCO = int.Parse(cbBancos.SelectedValue.ToString());
