...
{
    public partial class EntradaCampoXmayor : Form
    {
       string DNI        = "";
        string NOMBRE    = "";
        string APELLIDO  = "";
        ...
        public   EntradaCampoXmayor()
        {
            DNI       = "999999999";
            NOMBRE    = VGlobales.vp_role;
            APELLIDO  = VGlobales.vp_role;
            NRO_SOCIO = 0;
            NRO_DEP   = 0;
             LEGAJO   = 0;
            TIPO      = "TOT";
...

        {
            ...
            if (reintegro)
            {
                ...
                lnkIntercirculo.Visible = false;
                lnkInvitado.Visible = false;
        ...
        private void ControlReintegros()

        {
            int Positivos = 0;
            int Negativos = 0;

             Positivos = lista.Where(x => x.Monto > 0).Count();
             Negativos = lista.Where(x => x.Monto < 0).Count();
...

             {
                 ...
                 this.ActualizoPersonas();
                 throw new Exception("no puede haber reintegros e ingresos en un mismo Ticket, se reinicia la operacion");
        ...
        private void InviPleta_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            try
            {
        ...
        private void Pago_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DialogResult dr = MessageBox.Show("Ingreso Exitoso, imprimir Cupon ", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        {
            entradaCampoService.Imprimir(Socio, Socio_Pileta, Socio_Estacionamiento, Invitado, Invitado_Pileta, Invitado_Estacionamiento, Intercirculo, Intercirculo_Pileta, Intercirculo_Estacionamiento, Menor, Discapacitado, Discapacitado_Acompa, entradaCampoService.GetMaxID_ROL(DNI), DNI + "-" + APELLIDO + "," + NOMBRE, TIPO, false,false,true,"",0,false);
            entradaCampoService.Imprimir(Socio, Socio_Pileta, Socio_Estacionamiento, Invitado, Invitado_Pileta, Invitado_Estacionamiento, Intercirculo, Intercirculo_Pileta, Intercirculo_Estacionamiento, Menor, Discapacitado, Discapacitado_Acompa, entradaCampoService.GetMaxID_ROL(DNI), DNI + "-" + APELLIDO + "," + NOMBRE, TIPO, false, false, false,"",0,false);
        ...
        private void cbReintegro_CheckedChanged(object sender, EventArgs e)
        {
            if (cbReintegro.Checked)
            {
                esReintegro = true;
        ...
        {
            try { this.NuevoItem(entradaCampoService.EntradaInvi, 4); }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
