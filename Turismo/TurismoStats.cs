...
where B.TIPO='HOT' and V.BONO=B.ID and H.ID=V.HOTEL and B.FE_BONO > '" + Desde + "' AND B.FE_BONO < '" + Hasta + "'";
        ...
        public List<Memoria> getSalidas(string Desde, string Hasta)
        {


            List<Memoria> lista = new List<Memoria>();

            string QUERY = @"select  B.ID_ROL,B.FE_BONO,B.APELLIDO,B.NOMBRE ,
                             L.DESCRIPCION,BTP.CANTIDAD,
(select Sum(T.Monto) from pagos_bono T where T.TipoPago <>5 and T.BONO=B.ID)CONTADO,
(select Sum(T.Monto) from pagos_bono T where T.TipoPago =5 and T.BONO=B.ID)PLANILLA ,
 (select Count(T.ID) from pagos_bono T where T.TipoPago =5 and T.BONO=B.ID)CUOTAS,B.SALDO_INICIAL,B.OBS,B.PAGO,B.ID
from bono_turismo B,  Bono_Turismo_Pasajes BTP, Localidad L
where B.ID = BTP.BONO and  BTP.DESTINO= L.LOCALIDADID AND B.TIPO='PAS' and B.FE_BONO > '" + Desde + "' AND B.FE_BONO < '" + Hasta + "'";
            DataRow[] foundRows;
            foundRows = dlog.BO_EjecutoDataTable(QUERY).Select();
