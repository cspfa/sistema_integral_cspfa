      ...
      public List<Hotel_Stats_Ocupacion> getStats(DateTime Desde , DateTime Hasta )
       {
           ...
           Hasta = new DateTime(Hasta.Year, Hasta.Month, Hasta.Day, 0, 0, 0);

           Dias = (Hasta - Desde).Days;
           if (Dias == 0)
               Dias = 1;
           if (Hasta.Day != Desde.Day && Dias == 1) // para que sea exacto en cantidad de dias
           {
               Dias = Dias + 1;
           }
           ...
           if (foundRows.Length > 0)
           {
               while (I < foundRows.Length - 1)
               {
                   ...
                   hso.Diferencia = Int32.Parse(hso.Capacidad) - Int32.Parse(hso.Vendido);
                   lista.Add(hso);
